<?php
	...
	function users_is_phone_already_registered($phone){

		$enc_phone = AddSlashes($phone);

		$row = db_single(db_fetch("SELECT id FROM users WHERE phone='{$enc_phone}' AND deleted=0"));
	...
	function users_send_confirmation($phone){
	   
		$message = "Thanks for signing up for Object Phone. To confirm, reply DESIGN and receive a daily object, tailored just for you.\n\nText HELP for help, STOP to unsubscribe.";
		
		$ret = twilio_send_sms($phone, $message);

		header("location: {$GLOBALS['cfg']['abs_root_url']}success/");
		exit();
